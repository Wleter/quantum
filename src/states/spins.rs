...
impl SpinOperators {
    ...
    pub fn ladder_plus(dspin_bra: DoubleSpin, dspin_ket: DoubleSpin) -> f64 {
        if dspin_bra.0 == dspin_ket.0 && dspin_bra.1 == dspin_ket.1 + 2 {
            (((dspin_ket.0 * (dspin_ket.0 + 2)) as i32 - dspin_bra.1 * dspin_ket.1) as f64).sqrt() / 2.0
        } else {
            0.0
        }
    }
    ...
    pub fn dot(dspin1_braket: (DoubleSpin, DoubleSpin), dspin2_braket: (DoubleSpin, DoubleSpin)) -> f64 {
